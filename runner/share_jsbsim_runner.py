    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.buffer_size):
                #actor网络生成动作，动作概率，rnn等critic网络生成价值
                ...
                # insert data into buffer将数据插入到buffer
                self.insert(data)

            # compute return计算回报  
            self.compute()

            #update network更新网络；转到train的定义
            train_infos = self.train()

            # post process计算总的次数，传递日志
